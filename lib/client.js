...
Client.prototype._recordStats = function (res, feedName) {
  ...
  if (this.stats) {
    prefix = getStatsName(this.name, feedName);

    this.stats.increment(prefix + '.requests');
    this.stats.increment(prefix + '.responses.' + res.statusCode);
    this.stats.timing(prefix + '.response_time', res.elapsedTime);
  }
};
...
Client.prototype.get = function (url, opts, cb) {
  ...
  this._requestWithCircuitBreaker(GET, url, opts, function (err, body) {
    cb(err, body);
  });
};
...
Client.prototype.post = function (url, requestBody, opts, cb) {
  ...
  this._requestWithCircuitBreaker(POST, url, opts, function (err, body) {
    cb(err, body);
  });
};
...
Client.prototype.put = function (url, requestBody, opts, cb) {
  ...
  this._requestWithCircuitBreaker(PUT, url, opts, function (err, body) {
    cb(err, body);
  });
};
...
Client.prototype.delete = function (url, opts, cb) {
  ...
  this._requestWithCircuitBreaker(DELETE, url, opts, function (err, body) {
    cb(err, body);
  });
};
