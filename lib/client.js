...
const REQUESTS_LIMIT = process.env.NITRO_REQUESTS_LIMIT;
const LIMIT_INTERVAL = process.env.NITRO_LIMIT_INTERVAL;
...
Client.prototype._requestImpl = function (method, url, opts, cb) {
  var client = this;
...
Client.prototype._rateLimitedRequest = limit(function (client, method, url, opts, cb) {
  client._requestImpl(method, url, opts, cb);
}).to(REQUESTS_LIMIT).per(LIMIT_INTERVAL);

Client.prototype._request = function (method, url, opts, cb) {
  if(REQUESTS_LIMIT && LIMIT_INTERVAL) {
    this._rateLimitedRequest(this, method, url, opts, cb);    
  } else {
    this._requestImpl(method, url, opts, cb);  
  }
};
